...
We are able to genotype only 52 cells for STAG2 and IDH1.

```{r}
gene_interest <- c('IDH1', 'STAG2')
subset = get_cellid_full_genotype(genotype_file, variants, gene_interest)
```


### Effect of IDH1 mutation (MM vs WM)

We selected cells where IDH1 is either wild-type or mutated, while STAG2 is mutated. After matching genotyped cells with those passing scRNA-seq filters, we got basically no cells.
...
We identified 163 cells with SRSF2 mutations and 72 cells with SRSF2 WT.  The table below presents the number of cells in each compartment after filtering for cells with available RNA expression data. Since the majority of cells belong to the tumor compartment and only a small number are from the TME, we focused our analysis solely on the tumor compartment.
...
## STAG2 MUT vs  STAG2 WT 

We identified 300 cells with STAG2 mutations and 38 cells with STAG2 WT.  The table below presents the number of cells in each compartment after filtering for cells with available RNA expression data. Since the majority of cells belong to the tumor compartment and only a small number are from the TME, we focused our analysis solely on the tumor compartment.

```{r}
# Extract mutated STAG2 cells
gene_interest <- c('STAG2')
mutation_status1 = c('MUT')
subset = get_cellid_full_genotype(genotype_file, variants, gene_interest)
cells1 = subset$cell[subset$STAG2 == "MUT"]

mutation_status2 = c("WT")
cells2 = subset$cell[subset$STAG2 == "WT"]
